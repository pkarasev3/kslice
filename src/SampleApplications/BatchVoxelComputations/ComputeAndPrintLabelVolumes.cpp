...
int main( int argc, char **argv)
{
  ...
  if( argc >= 2 ) {
    ...
    for( int k = kstart; k < argc; k++ )
    {
      ...
      vector<double> minXYZ(3);
      vector<double> maxXYZ(3);

      // now get the min/max extents of nonzero portions after median filtering
      vrcl::getXYZExtentsAsString(spacing,img,volume_info[k-1].extents_string,minXYZ,maxXYZ,true /* number only!*/ );
      double center_line_y_axis = (maxXYZ[0]+minXYZ[0])/2.0;


      // =============================== part 1: filter for the volume measurements

      // make it a ushort and bag it
      img    = image2ushort( imgReader->GetOutput() );
      ...
      if( z_factor > 1.0 ) {
        ...
        if( bLeftRightSplit ) {
          vrcl::getVolumeAsString(spacing_new,imageDataTmp, volume_info[k-1].volume_Lstring,true /* number only!*/,"left",center_line_y_axis);
          vrcl::getVolumeAsString(spacing_new,imageDataTmp, volume_info[k-1].volume_Rstring,true /* number only!*/,"right",center_line_y_axis);
        }
